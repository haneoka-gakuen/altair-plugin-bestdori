...
import { bestdoriEditorAudioUsage, type BestdoriEditorAudioUsage } from "./assets.js";
import {
...
export type AltairBestdoriResourceReference = BestdoriResourceInsertDescriptor | { readonly kind: "unavailable" };
...
const requestServer = (request: ResourceBrowserRequest, fallback: BestdoriServer): BestdoriServer =>
...
const acceptedKinds = (node: BestdoriResourceFileNode): readonly string[] =>
  Object.freeze(
    node.media.kind === "image"
      ? ["background", "still", "frame", "image"]
      : node.media.kind === "audio"
        ? [node.insert?.kind === "asset" ? bestdoriEditorAudioUsage(node.insert.reference.bundlePath) : "se", "audio"]
...
const providerPath = (path: readonly string[]): readonly string[] => Object.freeze([...rootPath, ...path]);
  ...
  return Object.freeze({
    ...
    ...(node.preview?.kind === "audio"
      ...
      : node.preview
        ...
        : {}),
    acceptedKinds: kinds,
    available: Boolean(node.insert) && (request.acceptedKinds.length === 0 || requestAccepts(kinds, request)),
...
const audioUsage = (
  ...
  file.reference.kind === "asset" && file.reference.reference.mediaKind === "audio"
    ? bestdoriEditorAudioUsage(file.reference.reference.bundlePath)
    : undefined;

const requestedAudioUsages = (request: ResourceBrowserRequest): readonly BestdoriEditorAudioUsage[] => {
  const kinds = [
    request.preferredKind && request.acceptedKinds.includes(request.preferredKind) ? request.preferredKind : undefined,
    ...request.acceptedKinds,
  ];
  return Object.freeze(
    [...new Set(kinds)].filter(
      (kind): kind is BestdoriEditorAudioUsage => kind === "bgm" || kind === "se" || kind === "voice",
    ),
  );
};

const requestAccepts = (fileKinds: readonly string[], request: ResourceBrowserRequest): boolean => {
  if (request.acceptedKinds.length === 0) return true;
  const specificAudio = requestedAudioUsages(request);
  if (fileKinds.includes("audio") && specificAudio.length > 0) {
    return specificAudio.some((kind) => fileKinds.includes(kind));
  }
  return request.acceptedKinds.some((kind) => fileKinds.includes(kind));
};

const defaultKind = (file: ResourceBrowserFile<AltairBestdoriResourceReference>): string | undefined => {
...
const selectedKind = (
  ...
  if (inferredAudioUsage) {
    ...
    if (specificAudio.length > 0) {
      return specificAudio.includes(inferredAudioUsage) ? inferredAudioUsage : undefined;
    }
...
const neutralInsert = (resource: BestdoriEditorResourceInsert): ResourceBrowserInsert<AltairBestdoriResourceValue> => ({
  kind: resource.kind,
  key: resource.key,
...
export const createAltairBestdoriResourceProvider = (
  ...
  return Object.freeze({
    ...
    async open(file: ResourceBrowserFile<AltairBestdoriResourceReference>, request: ResourceBrowserRequest) {
      ...
      const visualKind =
        kind === "background" || kind === "still" || kind === "frame" || kind === "image"
          ? kind === "image"
            ? "background"
            : kind
          : undefined;
      const audioUsage = kind === "bgm" || kind === "se" || kind === "voice" ? kind : undefined;
